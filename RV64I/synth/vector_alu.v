`timescale 1ns / 1ps
// Elementwise vector ALU: 4 lanes of 32-bit elements (VLEN=128, SEW=32,
// LMUL=1 -- the only configuration this scoped RVV implementation
// supports). `operand2` is pre-selected at the top level to be either
// vs1's vector data (.vv), a scalar broadcast into all 4 lanes (.vx), or
// a sign-extended 5-bit immediate broadcast into all 4 lanes (.vi) --
...
// value), `cmp_bits` holds the 4 individual comparison booleans --
...
module vector_alu (
    ...
    input [127:0] operand2,
    input [4:0] v_op,
    output reg [127:0] vd_result,
    output reg [3:0] cmp_bits
);
