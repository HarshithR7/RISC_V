...
module riscv64_processor #(
    ...
    // Configurable vector width: SEW=32 always (see RV64I/README.md's
    // scoped RVV implementation), so LANES is both the number of 32-bit
    // elements per vector register and VLMAX (LMUL=1 always here too).
    // VLEN = LANES*32 is derived, not set directly, so the two can never
    // disagree. Default (4 -> VLEN=128) matches every prior test/README
    // number in this project; LANES should stay even (VLEN a multiple of
    // 64) since data_memory's vector port is doubleword-granular.
    parameter LANES = 4
)(
    input clk,
    input reset,
    ...
    localparam VL_BITS = $clog2(LANES + 1);  // enough bits to hold 0..LANES
    ...
    wire [1:0] v_form;
    wire [4:0] v_imm;
    wire [VLEN-1:0] vreg_read_data1, vreg_read_data2, v0_data, v_write_back_data, valu_result;
    wire [LANES-1:0] valu_cmp_bits;
    ...
    fp_register_file fp_regfile (
        ...
        .read_reg1(rs1), .read_reg2(rs2), .read_reg3(rs3), .write_reg(rd),
        .write_data(fp_write_back_data),
        .read_data1(fp_read_data1), .read_data2(fp_read_data2), .read_data3(fp_read_data3)
    );

    fpu fpu_module (
        .a(fp_read_data1), .b(fp_read_data2), .c(fp_read_data3),
        .int_a(read_data1),
        .fp_op(fp_op), .is_double(is_double), .word_int_src(word_int_src),
        .fresult(fpu_fresult), .iresult(fpu_iresult)
    );
    ...
    // ---- RVV (scoped: SEW=32, LMUL=1 only; VLEN=LANES*32 is a real
    // synthesis-time parameter -- "configurable vector width" in
    // RV64I/README.md) ---------------------------------------------------
    ...
    vector_register_file #(.LANES(LANES), .VLEN(VLEN)) vregfile (
        .clk(clk), .reset(reset),
    ...
    // integer value broadcast into all LANES lanes (.vx), or the
    // instruction's 5-bit immediate, sign-extended then broadcast (.vi).
    wire [31:0] v_imm_sext = {{27{v_imm[4]}}, v_imm};
    reg [VLEN-1:0] v_operand2;
    always @(*) begin
        case (v_form)
            2'd0: v_operand2 = vreg_read_data2;                          // .vv
            2'd1: v_operand2 = {LANES{read_data1[31:0]}};                // .vx
            2'd2: v_operand2 = {LANES{v_imm_sext}};                      // .vi
            default: v_operand2 = {VLEN{1'b0}};
        endcase
    end

    vector_alu #(.LANES(LANES), .VLEN(VLEN)) valu_module (
    ...
    // it to min(AVL, VLMAX=LANES), or VLMAX itself when rs1=x0 (the
    // spec's "set vl to the maximum" idiom, which reading x0 as a value
    // of 0 would otherwise misinterpret as "set vl to 0").
    reg [VL_BITS-1:0] vl_reg;
    wire vsetvli_use_vlmax = (rs1 == 5'd0);
    wire [VL_BITS-1:0] vsetvli_new_vl = vsetvli_use_vlmax ? LANES[VL_BITS-1:0] :
                                 (read_data1 > LANES) ? LANES[VL_BITS-1:0] : read_data1[VL_BITS-1:0];
    always @(posedge clk or posedge reset) begin
        if (reset) vl_reg <= LANES[VL_BITS-1:0];
        else if (is_vsetvli) vl_reg <= vsetvli_new_vl;
    end

    // Per-lane "below vl" mask, built generically so it scales with
    // LANES: tail_mask[i] = 1 when lane i is an active element of this
    // vl (i.e. i < vl), both as a LANES-bit mask and replicated out to a
    // full VLEN-bit per-lane byte mask for use directly against 32-bit-
    // wide vector data.
    wire [LANES-1:0] tail_mask;
    wire [VLEN-1:0] vle_lane_mask;
    genvar tm_i;
    generate
        for (tm_i = 0; tm_i < LANES; tm_i = tm_i + 1) begin : tail_mask_gen
            assign tail_mask[tm_i] = (vl_reg > tm_i);
            assign vle_lane_mask[tm_i*32 +: 32] = {32{tail_mask[tm_i]}};
        end
    endgenerate

    // vle32.v: zero-fill (tail-agnostic) any lane at or beyond vl.
    wire [VLEN-1:0] vle_result = vmem_read_data & vle_lane_mask;
    ...
    // always-on vmem_read_data port, since data_memory's write commits
    // the full VLEN bits every time).
    wire [VLEN-1:0] vse_write_data = (vreg_read_data1 & vle_lane_mask) | (vmem_read_data & ~vle_lane_mask);
    ...
    wire [LANES-1:0] full_lane_mask = v_vm ? tail_mask : (tail_mask & v0_data[LANES-1:0]);
    wire [VLEN-1:0] lane_mask_full;
    genvar lm_i;
    generate
        for (lm_i = 0; lm_i < LANES; lm_i = lm_i + 1) begin : lane_mask_gen
            assign lane_mask_full[lm_i*32 +: 32] = {32{full_lane_mask[lm_i]}};
        end
    endgenerate
    wire [VLEN-1:0] varith_result = valu_result & lane_mask_full;
    // Compare results are 1 bit/lane, packed into vd's low LANES bits (a
    // mask register); the rest of vd is agnostic (zeroed).
    wire [VLEN-1:0] vcmp_result = {{(VLEN-LANES){1'b0}}, (valu_cmp_bits & full_lane_mask)};
    ...
    always @(*) begin
        ...
        for (r_i = 0; r_i < LANES; r_i = r_i + 1) begin
    ...
    data_memory #(
        .DMEM_FILE(DMEM_FILE), .DMEM_WORDS(DMEM_WORDS), .VLEN(VLEN)
    ) dmem (
    ...
    assign write_back_data = is_sc ? (sc_success ? 64'd0 : 64'd1) :
                              is_vsetvli ? {{(64-VL_BITS){1'b0}}, vsetvli_new_vl} :
    ...
    assign v_write_back_data = is_vle ? vle_result :
                                is_vreduce ? {{(VLEN-32){1'b0}}, vreduce_acc} :
